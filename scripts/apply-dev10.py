...
path = Path("src/ha-floorplan-zone-card.js")
...
replace_once(
    ...
    this._imageKey = key;
    const token = ++this._imageResolveToken;''',
    "reset changed image aspect ratio",
)
...
package["version"] = "0.1.0-dev.10"
package_path.write_text(json.dumps(package, indent=2) + "\n")

Path("DEV10_NOTES.md").write_text('''# 0.1.0-dev.10 implementation notes
...
marker = 'test("raster zoom uses layout sizing instead of transform scaling"'
